...
- **No "squeeze/pop" scale animations on mount, unmount, or visibility transitions** unless the user explicitly asks for one. Cards, banners, popovers, dialogs, dropdowns, sheets, toasts, list items, etc. enter and leave with `opacity` (+ optional `y`/`x` for slide) — they do **not** pop in from `scale: 0.95-0.98` or shrink out to a smaller scale on exit. The cumulative scale-pulse across the app reads as twitchy and infantilises the UI. Still allowed: `scale` on `whileHover` / `whileTap` press feedback, on drop-target `isOver` indicators, and on hero/marketing entrances when the user has explicitly asked for that kind of motion.
