...
import { getPublicUserProfileAction } from '../api/get-public-user-profile-action';
import type { PublicUserProfile } from '../model/types';

type UserCoverProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  userId: string;
  /**
   * When the parent already holds the profile (server-rendered detail page,
   * pre-populated list, …), pass it through to skip the client-side fetch.
   */
  initialProfile?: PublicUserProfile;
  /**
   * Local file preview (e.g. picked from `<input type="file">` before save).
   * Takes precedence over the fetched cover and renders immediately through
   * a managed object URL — no network involved.
   */
...
export function UserCover({
  ...
  const t = useTranslations('user-profile.cover');
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const result = await getPublicUserProfileAction(userId);
      if (!result.ok) throw new Error(result.reason);
      return result.profile;
    },
    initialData:
      initialProfile && initialProfile.id === userId ? initialProfile : undefined,
    staleTime: 60_000,
  });
  ...
  if (previewUrl) {
    ...
    body = (
      ...
      >
        <div className="flex max-w-[20rem] flex-col items-center gap-2 px-4 py-3 text-center">
          <div className="flex size-9 items-center justify-center rounded-full bg-background/80 text-foreground/70 shadow-sm ring-1 ring-foreground/10">
            {isNotFound ? (
