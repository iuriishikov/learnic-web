...
type NavKey = 'dashboard' | 'courses' | 'students' | 'analytics' | 'revenue';
...
export function StudioHeader({
  ...
  const t = useTranslations('test-studio-mode.teach');
    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background">
      <div className="mx-auto flex h-[72px] w-full max-w-[1440px] items-center justify-between gap-3 px-4 md:gap-6 md:px-8">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="default"
            className="hidden h-10 gap-2 rounded-lg bg-brand text-brand-foreground shadow-xs hover:bg-brand/90 md:inline-flex"
          >
          ...
          <Button
            ...
            aria-label={tRoot('learn.notifications')}
          >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" sideOffset={8} className="w-60">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={onSwitchToLearn}>
                <BookOpenIcon className="size-4 text-brand" />
                <span className="font-semibold text-brand">
                  {t('menu.switchToLearn')}
                </span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem>
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            >
              ...
              <span className="mb-6 inline-flex w-fit items-center gap-1 rounded-md bg-brand/10 px-2 py-0.5 text-xs font-semibold text-brand">
                {tRoot('studioBadge')}
              </span>
              ...
              <Button
                variant="default"
                className="mt-6 h-11 w-full gap-2 rounded-lg bg-brand text-brand-foreground hover:bg-brand/90"
              >
                <PlusIcon className="size-4" />
                {t('createCourse')}
              </Button>
